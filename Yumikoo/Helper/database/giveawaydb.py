    ...
    def give_info(self, group_id=0, u_id=0):
        with INSERTION_LOCK:
            if u_id and group_id:
                if curr := self.db.find_one({"where": group_id, "user_id": u_id}):
                    return curr
                if curr := self.db.find_one(
                    {"chat_id": group_id, "user_id": u_id}
                ):
                    return curr
            elif u_id:
                if curr := self.db.find_one({"user_id": u_id}):
                    return curr
            elif group_id:
                if curr := self.db.find_one({"where": group_id}):
                    return curr
                if curr := self.db.find_one({"chat_id": group_id}):
                    return curr
        return False

    def is_vote(self, group_id):
        with INSERTION_LOCK:
            return bool(curr := self.db.find_one({"where": group_id}))

    def start_vote(self, user_id, start=1):
        with INSERTION_LOCK:
            if curr := self.db.find_one({"user_id": user_id}):
