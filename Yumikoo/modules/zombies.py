...
@Yumikoo.on_message(filters.command(["zombies","clean"]))
async def remove(client, message):
  global stopProcess
  try: 
    try:
      sender = await Yumikoo.get_chat_member(message.chat.id, message.from_user.id)
      has_permissions = sender.privileges
    except:
      has_permissions = message.sender_chat  
    if has_permissions:
      bot = await Yumikoo.get_chat_member(message.chat.id, "self")
      if bot.status == ChatMemberStatus.MEMBER:
        ...
        if len(chatQueue) > 30 :
          ...
          if message.chat.id in chatQueue:
            ...
            deletedList = []
            async for member in Yumikoo.get_chat_members(message.chat.id):
            ...
            if lenDeletedList == 0:
              ...
              temp = await Yumikoo.send_message(message.chat.id, f"🧭 | ᴛᴏᴛᴀʟ ᴏғ {lenDeletedList} ᴅᴇʟᴇᴛᴇᴅ ᴀᴄᴄᴏᴜɴᴛs ʜᴀs ʙᴇᴇɴ ᴅᴇᴛᴇᴄᴛᴇᴅ.\n🥀 | ᴇsᴛɪᴍᴀᴛᴇᴅ ᴛɪᴍᴇ: {processTime} sᴇᴄᴏɴᴅs ғʀᴏᴍ ɴᴏᴡ.")
              if stopProcess: stopProcess = False
              while len(deletedList) > 0 and not stopProcess:   
                deletedAccount = deletedList.pop(0)
                try:
                  await Yumikoo.ban_chat_member(message.chat.id, deletedAccount.id)
                except Exception:
                  pass  
                k+=1
...
@Yumikoo.on_message(filters.command(["admins","staff"]))
async def admins(client, message):
  try: 
    adminList = []
    ownerList = []
    async for admin in Yumikoo.get_chat_members(message.chat.id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
    ...
    await Yumikoo.send_message(message.chat.id, text2)
  except FloodWait as e:
    await asyncio.sleep(e.value)       

# ------------------------------------------------------------------------------- #

@Yumikoo.on_message(filters.command("bots"))
async def bots(client, message):  
  try:    
    botList = []
    async for bot in Yumikoo.get_chat_members(message.chat.id, filter=enums.ChatMembersFilter.BOTS):
    ...
    while len(botList) > 1:
      ...
      await Yumikoo.send_message(message.chat.id, text3)
  except FloodWait as e:
    await asyncio.sleep(e.value)
