...
@Yumikoo.on_message(filters.command("bug"))
...
@Yumikoo.on_callback_query(filters.regex("close_send_photo"))
async def close_send_photo(_,  query :CallbackQuery):
    is_admin = await Yumikoo.get_chat_member(query.message.chat.id, query.from_user.id)
