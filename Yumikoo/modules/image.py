...
@Yumikoo.on_message(filters.command(["draw","create","imagine","dream"]))
async def draw(_: Yumikoo, m: t.Message):
    global Database
    prompt = getText(m)
    if prompt is None:
...
@Yumikoo.on_callback_query(filters.regex(pattern=r"^d.(.*)"))
async def selectModel(_:Yumikoo,query:t.CallbackQuery):
    global Database
